...
int main()
{
    ...
    MTL::Library* adder_lib = gpu->newLibrary(NS::String::string(ADDER_SRC, NS::UTF8StringEncoding), nullptr, &compile_error);
    if (adder_lib == nullptr) assert(false);
    MTL::Function* adder_func = adder_lib->newFunction(NS::String::string("add_arrays", NS::UTF8StringEncoding));
    ...
    MTL::ComputePipelineState* pipeline = gpu->newComputePipelineState(adder_func, &pipeline_error);
    if (pipeline == nullptr) assert(false);
    NS::UInteger max_threads_per_group = pipeline->maxTotalThreadsPerThreadgroup();
    std::cout << "Max threads per threadgroup: " << max_threads_per_group << std::endl;
    ...
    MTL::Buffer* a_buffer = gpu->newBuffer(BUFFER_SIZE, MTL::ResourceStorageModeManaged);
    MTL::Buffer* b_buffer = gpu->newBuffer(BUFFER_SIZE, MTL::ResourceStorageModeManaged);
    MTL::Buffer* res_buffer = gpu->newBuffer(BUFFER_SIZE, MTL::ResourceStorageModeManaged);
    ...
    std::cout << "Done with all GPU work, now trying on CPU"
              << std::endl;
